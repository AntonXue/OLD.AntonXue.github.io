...
\begin{algorithm}[H]
  ...
  \While{\(v \leftarrow \next()\) succeeds}{
    ...
    \tcc*[r]{There is something for \(v\) to merge with}
    \uIf{\(\abs{M} > 0\)}{
      \(V \leftarrow V \cup \{v\}\)\;
    ...
    \tcc*[r]{There is a ``most recent dependency'' of \(v\) to merge with}
      \uIf{\(u^\star\) exists}{
        \(V \leftarrow V \cup \{v\}\)\;
        \(E \leftarrow E \cup \{(u^\star, v)\}\)\;
        \(L \leftarrow L \cup \{v\}\)\;

      \tcc*[r]{\(v\) is to be parallel unioned with the existing graph}
      }\Else{
        \(V \leftarrow V \cup \{v\}\)\;
        \(L \leftarrow L \cup \{v\}\)
      }
