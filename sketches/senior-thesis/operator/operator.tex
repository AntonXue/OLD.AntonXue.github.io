...
We remark that normed vector spaces that are complete with repsect to their
...
Because of this, we use the term linear space to denote spaces
...
be seen as a directed graph on the states \(Q\),
...
denotes the set of letters that will transition state \(q_i\) to \(q_j\).
...
Note that the above is also the definition of a semiring,
and here the string space is the power set of all strings generated
...
long strings, rather than just single symbols.
...
The elements of a string measure space consists of finite strings,
while the \(\sigma\)-algebra generated consists of sets of finite strings.
A measure \(\type{\lambda}{\sigma\parens{\Sigma^\star}}{\Rz}\)
as defined before can also be used to define a metric.
