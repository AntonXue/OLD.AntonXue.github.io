...
Another way to measure the distance between two languages is
with respect to the size of the smallest NFA that separates them.
This is known as the string separation problem.

More formally, given two sets of strings
\(P\) and \(N\),
...
A metric can then be defined as:
\begin{align*}
  d\parens{P, N} = \frac{1}{2^{\abs{\mcal{A}}}}
\end{align*}
Where \(\abs{\mcal{A}}\) counts the number of states of \(\mcal{A}\).
Note that minimally separating NFAs need not be unique.
